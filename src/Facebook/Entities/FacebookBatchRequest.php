...
class FacebookBatchRequest extends FacebookRequest implements \IteratorAggregate
{
   ...
   */
  public function __construct(array $requests = [], AccessToken $fallbackAccessToken = null)
  {
    parent::__construct(
      '/',
      'POST',
  ...
  protected function processRequests(array $requests = [])
  {
    ...
    foreach($requests as $request) {
      if (!$this->accessToken && !$request->getAccessToken()) {
        throw new FacebookSDKException('There is a request without access token in a batch request that do not have a default one');
      }

      $this->requests[] = $request;

      $this->params['batch'][] = $this->processRequest($request);
    }
  }
   ...
   */
  protected function processRequest(FacebookRequest $request)
  {
    $batch = [
      'method' => $request->getMethod(),
      'relative_url' => ltrim($request->getEndpoint(), '/'),
    ];
    ...
    if (!empty($headers)) {
      $batch['headers'] = $compiledHeaders;
    }

    if ($request->getAccessToken()) {
      $batch['access_token'] = $request->getAccessToken()->getValue();
    }
    ...
    if ($request instanceof FacebookBatchedRequest) {
      $batch = array_merge($batch, $this->processBatchedRequest($request));
    }

    return $batch;
  }

  protected function processBatchedRequest(FacebookBatchedRequest $request)
  {
    $batch = [];
    ...
    if ($dependsOn) {
      if (!$this->hasRequest($dependsOn)) {
        throw new FacebookSDKException(sprintf('FacebookBatchedRequest named "%s" don\'t exists', $dependsOn));
      }

      $batch['depends_on'] = $dependsOn;
    }

    if (!$request->isOmitResponseOnSuccess()) {
