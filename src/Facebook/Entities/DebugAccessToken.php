...
class DebugAccessToken extends AccessToken
{
  ...
  protected function parseResponse(FacebookResponse $response)
  {
    ...
    if (isset($body['data']['app_id'])) {
      $this->appId = $body['data']['app_id'];
    }

    if (isset($body['data']['application'])) {
      $this->appName = $body['data']['application'];
    }

    if (isset($body['data']['expires_at'])) {
      $this->expiresAt = new \DateTime();
      $this->expiresAt->setTimestamp($body['data']['expires_at']);
    }

    if (isset($body['data']['is_valid'])) {
      $this->isValid = (bool)$body['data']['is_valid'];
    }

    if (isset($body['data']['issued_at'])) {
      $this->issuedAt = new \DateTime();
      $this->issuedAt->setTimestamp($body['data']['issued_at']);
    }

    if (isset($body['data']['scopes'])) {
      $this->scopes = $body['data']['scopes'];
    }
