...
`GraphNode`'s are obtained from a [`Facebook\FacebookResponse`](/docs/reference/FacebookResponse.md) object which represents an HTTP response from the Graph API.
...
```
Returns the `birthday` property for the user as a [`Facebook\GraphNodes\Birthday`](/docs/reference/Birthday.md) if present.
...
| `global_brand_parent_page` | [`Facebook\GraphNodes\GraphPage`](#graphpage-instance-methods) |
