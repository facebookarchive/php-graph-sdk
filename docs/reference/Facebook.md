...
Allows you to overwrite the default Graph version number set in `Facebook\Facebook::DEFAULT_GRAPH_VERSION`. Set this as a string as it would appear in the Graph url, e.g. `v2.6`. Defaults to the [latest version of Graph](https://developers.facebook.com/docs/apps/changelog).
...
If you wish to write your own HTTP client, you can code your HTTP client to the `[Facebook\HttpClients\FacebookHttpClientInterface](/docs/reference/FacebookHttpClientInterface.md)` and set this value to an instance of your custom client.
...
If you wish to write your own persistent data handler, you can code your persistent data handler to the `[Facebook\PersistentData\PersistentDataInterface](/docs/reference/PersistentDataInterface.md)` and set the value of `persistent_data_handler` to an instance of your custom handler.
...
You can write your own CSPRSG that implements the `[Facebook\PseudoRandomString\PseudoRandomStringGeneratorInterface](/docs/reference/PseudoRandomStringGeneratorInterface.md)` and set the value of `pseudo_random_string_generator` to an instance of your custom generator.
...
```
Returns the instance of [`Facebook\FacebookClient`](/docs/reference/FacebookClient.md) for the instantiated service.

## getOAuth2Client()
```php
public Facebook\Authentication\OAuth2Client getOAuth2Client()
```
Returns an instance of [`Facebook\Authentication\OAuth2Client`](/docs/reference/OAuth2Client.md).
...
```
Returns an instance of [`Facebook\Url\UrlDetectionInterface`](/docs/reference/UrlDetectionInterface.md).

## getDefaultAccessToken()
```php
public Facebook\Authentication\AccessToken|null getDefaultAccessToken()
```
Returns the default fallback [`AccessToken`](/docs/reference/AccessToken.md) entity that is being used with every request to Graph. This value can be set with the configuration option `default_access_token` or by using `setDefaultAccessToken()`.

## setDefaultAccessToken()
```php
public setDefaultAccessToken(string|Facebook\AccessToken $accessToken)
```
Sets the default fallback access token to be use with all requests sent to Graph. The access token can be a string or an instance of [`AccessToken`](/docs/reference/AccessToken.md).
...
[See a full batch example](/docs/reference/howto/example_batch_request.md).
...
Returns a [`Facebook\Helpers\FacebookRedirectLoginHelper`](/docs/reference/FacebookRedirectLoginHelper.md) which is used to generate a "Login with Facebook" link and obtain an access token from a redirect.
...
Returns a [`Facebook\Helpers\FacebookJavaScriptHelper`](/docs/reference/FacebookJavaScriptHelper.md) which is used to access the signed request stored in the cookie set by the SDK for JavaScript.
...
Returns a [`Facebook\Helpers\FacebookCanvasHelper`](/docs/reference/FacebookCanvasHelper.md) which is used to access the signed request that is `POST`ed to canvas apps.
...
Returns a [`Facebook\Helpers\FacebookPageTabHelper`](/docs/reference/FacebookPageTabHelper.md) which is used to access the signed request that is `POST`ed to canvas apps and provides a number of helper methods useful for apps living in a page tab context.
...
Requests and returns the next page of results in a [`Facebook\GraphNodes\GraphEdge`](/docs/reference/GraphEdge.md) collection. If the next page returns no results, `null` will be returned.
